...
class FactorUCBArticleStruct:
    ...
    def updateParameters(self, user, click, userID):
        ...
        self.b2 += user.CoTheta.T[userID][self.context_dimension:] * (
            click - user.CoTheta.T[userID][:self.context_dimension].dot(
...
class FactorUCBUserStruct:
    ...
    def updateParameters(self, articles, clicks, userID):
        ...
        self.UserTheta = matrixize(
            np.dot(self.AInv, self.b), len(articles[0].V))
        self.CoTheta = np.dot(self.UserTheta, self.W)
        self.CCA = np.dot(
            np.dot(self.BigW, self.AInv), np.transpose(self.BigW))
    ...
    def getProb(self, alpha, alpha2, article, userID):
        if alpha == -1:
            alpha = 0.1 * np.sqrt(
                np.log(self.time + 1)) + 0.1 * (1 - 0.8**self.time)
            alpha2 = 0.1 * np.sqrt(
                np.log(article.time + 1)) + 0.1 * (1 - 0.8**article.time)
        ...
        var2 = np.sqrt(
            np.dot(
                np.dot(self.CoTheta.T[userID][self.context_dimension:],
                       article.A2Inv),
...
class FactorUCBAlgorithm:
    def __init__(self,
                 ...
                 init="zero",
                 window_size=1,
                 max_window_size=10):  # n is number of users
        ...
        self.USERS = FactorUCBUserStruct(context_dimension, latent_dimension,
                                         lambda_, n, W, init)
        self.articles = []
        for i in range(itemNum):
            # print (i)
            self.articles.append(
                FactorUCBArticleStruct(i, context_dimension, latent_dimension,
                                       lambda_, W, init))
    ...
    def decide(self, pool_articles, userID):
        ...
        for x in pool_articles:
            self.articles[
                x.
                id].V[:self.
                      context_dimension] = x.contextFeatureVector[:self.
                                                                  context_dimension]
            x_pta = self.USERS.getProb(self.alpha, self.alpha2,
                                       self.articles[x.id], userID)
    ...
    def updateParameters(self, articlePicked, click, userID):
        ...
        if len(self.window) % self.window_size == 0:
            ...
            if self.increase_window == True:
                self.window_size = min(self.window_size + 1,
