...
class UCB1_Article:
    ...
    def getClickProb(self):
        if len(self.X) == 0:
            ...
            if 1 not in self.Y:
                ...
                sample = np.array([1]).reshape(
                    1, -1)  # Reshpae to avoid error, 1-d feature
    ...
    def getAverageReturnProb(self, ReturnThreshold):
        try:
            self.averageReturnTime = self.totalReturnTime / float(
                self.PlayCount_return)
...
class r2_GLMUCB1Algorithm:
    def __init__(self, dimension, alpha, lambda_, FutureWeight,
                 ReturnThreshold, usealphaT, RankoneInverse):
    ...
    def decide(self, pool_articles, userID, pool_article_Arr):
        if userID not in self.users:
            ...
            self.users_click[userID] = LogisticUserStruct(
                featureDimension=self.dimension,
                userID=userID,
                lambda_=self.lambda_,
                RankoneInverse=self.RankoneInverse)
        ...
        ClickProb_arr = self.users_click[userID].getClickProb_arr(
        ...
        for x in pool_articles:
            ...
            if x_ID not in self.users[userID].ArticleDic:
                ...
                var = self.users[userID].getVar(x,
                                                self.users[userID].totalCount)
