...
class CLUBUserStruct(LinUCBUserStruct):
    ...
    def getProb(self, alpha, article_FeatureVector, time):
        ...
        var = np.sqrt(
            np.dot(
                np.dot(article_FeatureVector, self.CAInv),
...
class CLUBAlgorithm():
    ...
    def decide(self, pool_articles, userID):
        self.users[userID].updateParametersofClusters(self.clusters, userID,
                                                      self.Graph, self.users)
        maxPTA = float('-inf')
        articlePicked = None
    ...
    def updateGraphClusters(self, userID, binaryRatio):
        ...
        N_components, component_list = connected_components(
