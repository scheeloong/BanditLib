...
class Uniform_LinUCBAlgorithm(object):
    ...
    def decide(self, pool_articles, userID):
        ...
        for x in pool_articles:
            x_pta = self.USER.getProb(self.alpha,
...
class N_LinUCBAlgorithm:
    def __init__(self, dimension, alpha, lambda_,
                 init="zero"):  # n is number of users
        self.users = {}
        self.dimension = dimension
        self.alpha = alpha
    ...
    def decide(self, pool_articles, userID):
        if userID not in self.users:
            self.users[userID] = LinUCBUserStruct(self.dimension, self.lambda_,
                                                  self.init)
        maxPTA = float('-inf')
        articlePicked = None
...
class Hybrid_LinUCBUserStruct:
    ...
    def updateParameters(self, articlePicked_FeatureVector, click, userID):
        ...
        temp = np.dot(
            np.transpose(self.users[userID].B), self.users[userID].AInv)
...
class Hybrid_LinUCBAlgorithm(object):
    def __init__(self, dimension, alpha, lambda_, userFeatureList):
        self.dimension = dimension
        self.alpha = alpha
        self.USER = Hybrid_LinUCBUserStruct(dimension, lambda_,
                                            userFeatureList)
    ...
    def updateParameters(self, articlePicked, click, userID):
        self.USER.updateParameters(articlePicked.contextFeatureVector, click,
                                   userID)
