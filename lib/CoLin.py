    ...
    def getProb(self, alpha, article, userID):
        ...
        if ComputeAlpha_t:
            self.alpha_t = 0.01 * np.sqrt(
                np.log(
                    np.linalg.det(self.A) / float(
                        self.sigma * self.lambda_))) + np.sqrt(self.lambda_)
...
class SyCoLinUCBUserSharedStruct(CoLinUCBUserSharedStruct):
    def __init__(self, featureDimension, lambda_, userNum, W):
        CoLinUCBUserSharedStruct.__init__(self, featureDimension, lambda_,
                                          userNum, W)
        self.contextFeatureVectorMatrix = np.zeros(
            shape=(featureDimension, userNum))
        self.reward = np.zeros(userNum)

    def updateParameters(self, articlePicked, click, userID):
        self.contextFeatureVectorMatrix.T[
            userID] = articlePicked.contextFeatureVector
    ...
    def LateUpdate(self):
        ...
        current_A = np.zeros(
            shape=(featureDimension * self.userNum,
                   featureDimension * self.userNum))
        current_b = np.zeros(featureDimension * self.userNum)
        for i in range(self.userNum):
            X = vectorize(
                np.outer(self.contextFeatureVectorMatrix.T[i], self.W.T[i]))
            XS = np.outer(X, X)
            current_A += XS
...
class CoLinUCBAlgorithm:
    def __init__(self, dimension, alpha, lambda_, n, W,
                 update='inv'):  # n is number of users
