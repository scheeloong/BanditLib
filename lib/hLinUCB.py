...
class HLinUCBArticleStruct:
    ...
    def updateParameters(self, user, click):
        ...
        self.A2 += np.outer(user.U[self.context_dimension:],
                            user.U[self.context_dimension:])
        self.b2 += user.U[self.context_dimension:] * (
            click - user.U[:self.context_dimension].dot(
...
class HLinUCBAlgorithm:
    def __init__(self,
                 ...
                 init="zero",
                 window_size=1,
                 max_window_size=50):  # n is number of users
        ...
        for i in range(n):
            self.users.append(
                HLinUCBUserStruct(i, context_dimension, latent_dimension,
                                  lambda_, init))
        self.articles = []
        for i in range(itemNum):
            self.articles.append(
                HLinUCBArticleStruct(i, context_dimension, latent_dimension,
                                     lambda_, init))
            ...
            self.articles[
                ...
                id].V[:self.
                      context_dimension] = x.contextFeatureVector[:self.
                                                                  context_dimension]
            x_pta = self.users[userID].getProb(self.alpha, self.alpha2,
                                               self.articles[x.id])
